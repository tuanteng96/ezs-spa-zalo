...
const News = () => {
  ...
  return (
    <div className="bg-white mb-2.5">
      <NavLink to="/news" state={{ dataProps: data }} className="p-3 font-semibold text-app uppercase flex justify-between items-center">
        {isLoading ? <div className="h-4 bg-gray-200 rounded-full w-2/5"></div> : data[0].source.CateTitle2}
      ...
      <div className="px-3 pb-3">
        ...
        >
          ...
          {
            !isLoading && data.slice(0, 6).map((item, index) => (
              <SwiperSlide style={{ width: "40%" }} key={index}>
                <NavLink to={'/news/' + item.id} state={{ dataProps: item }} className="bg-white block aspect-0/7">
                  <div>
                    <ImageLazy wrapperClassName="aspect-0/7 !block" className="aspect-0/7 object-cover h-full" effect="blur" src={toAbsolutePath(item.source.Thumbnail)} />
                  </div>
                  <div className="absolute left-0 bottom-0 w-full h-3/6 bg-pattern"></div>
                  <div className="absolute w-full bottom-0 left-0 p-2 uppercase text-white font-semibold text-xs">
