    ...
    <div className="grid grid-cols-4 gap-y-4 bg-white px-3 pt-5 pb-3">
      {
        isLoading && Array(8).fill().map((_, index) => (
          <div className="flex flex-col items-center animate-pulse" key={index}>
      ...
      }
      {
        !isLoading && data.map((item, index) => (
          <NavLink to="/" className="flex flex-col items-center" key={index}>
            <ImageLazy wrapperClassName="aspect-square w-16 !block" className="rounded-full w-16 shadow-[2px_0px_0px_2px_#e4ad2f] aspect-square object-cover" effect="blur" src={toAbsolutePath(item.FileName)} />
            <div className="text-center mt-1.5 text-[11px] leading-[14px]" size="xxSmall">{item.Title}</div>
          </NavLink>
        ))
      }
