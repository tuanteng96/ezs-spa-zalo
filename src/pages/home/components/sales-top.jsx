      ...
      <div className="px-2 bg-white mb-3 py-2 flex">
        {
          isLoading && Array(3).fill().map((_, index) => (
            <div className={clsx('px-1 animate-pulse', index === 1 ? 'aspect-5/3 w-[43%]' : 'flex-1')}>
        ...
        {
          !isLoading && data.slice(0, 3).map((item, index) => (
            <NavLink to="/" className={clsx('px-1', index === 1 ? 'aspect-5/3 w-[43%]' : 'flex-1')} key={index}>
              <ImageLazy className="object-cover !block" effect="blur" src={toAbsolutePath(item.FileName)} />
            </NavLink>
          ))
        }
      </div>
