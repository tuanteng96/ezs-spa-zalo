...
const BookingServiceItem = ({ item }) => {
  ...
  const isCard = item.OsBook > 0 || item.OsDoing > 0 || item.OsNew > 0 || item.OsBH > 0
    ...
    <Controller
      ...
      render={({
        ...
        <div
          ...
          onClick={() => {
            let index = field.value && field.value.findIndex(x => Number(x) === item.ID)
            if (index > -1) {
              field.onChange(
                field.value.filter(
                  (x) => Number(x) !== item.ID
        ...
        >
          <div className="flex-1 pr-8">
            ...
            {item.SaleDecs && (
              <div dangerouslySetInnerHTML={{ __html: item.SaleDecs }}></div>
            )}
          </div>
          <div className={clsx("border-2 w-6 h-6 rounded-full flex items-center justify-center", field.value && field.value.includes(item.ID) && 'bg-app text-white')}>
            <Icon icon="zi-check" className={clsx('!w-[16px] !h-[18px] !text-[17px] transition', field.value && field.value.includes(item.ID) ? 'opacity-100' : 'opacity-0')} />
          </div>
        </div>
      )}
    />
              ...
              <>
                {
                  Array(5).fill().map((_, index) => (
                    <div className="px-3 py-3.5 rounded-sm cursor-pointer bg-white border-b" key={index}>
