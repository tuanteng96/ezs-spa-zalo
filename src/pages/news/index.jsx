    ...
    <Page className="page !pb-0" hideScrollbar>
      <div className="navbar fixed top-0 left-0 min-w-[100vw] max-w-[100vw] z-[999] bg-white shadow-3xl">
        <div className="w-2/3 relative flex items-center h-full pl-10">
          ...
          <Text.Title className="text-app">{isLoading ? "Đang tải ..." : data[0].source.CateTitle2}</Text.Title>
        </div>
      </div>
        ...
        <div className="p-3">
          {
            isLoading && Array(2).fill().map((_, index) => (
              <div className="bg-white animate-pulse mb-3 shadow-3xl rounded overflow-hidden last:mb-0" key={index}>
                <div>
          ...
          }
          {
            !isLoading && data.map((item, index) => (
              <NavLink to={'/news/' + item.id} state={{ dataProps: item }} className="bg-white shadow-3xl mb-3 block rounded overflow-hidden last:mb-0" key={index}>
                <div>
                  <ImageLazy wrapperClassName="aspect-square !block" className="w-full aspect-square object-cover" effect="blur" src={toAbsolutePath(item.source.Thumbnail)} />
                </div>
                <div className="p-3">
                  <Text.Title className="line-clamp-2 mb-2">{item.text}</Text.Title>
                  <div className="line-clamp-3 text-muted" dangerouslySetInnerHTML={{ __html: item?.source?.Desc }} />
                </div>
              </NavLink>
            ))
          }
