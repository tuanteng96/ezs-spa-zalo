...
const SkeletonServices = ({ total = 2, className = "p-3 grid grid-cols-1 gap-3" }) => {
  return (
          ...
          .map((_, index) => (
            <div className="bg-white" key={index}>
              ...
              <div className="p-3">
                ...
                <div className="flex border-t border-dashed py-3 flex justify-between">
...
const ListServices = ({ queryConfig }) => {
  ...
  const { data, fetchNextPage, isLoading, hasNextPage } = useInfiniteQuery({
    ...
    getNextPageParam: (lastPage) => {
      return lastPage.pi === lastPage.pcount || !lastPage.pcount ? undefined : lastPage.pi + 1
    }
  })

  const List = formatArray.useInfiniteQuery(
    data?.pages, "lst"
  )
  ...
  return (
    <div id="scrollableProducts" className="grow no-scrollbar overflow-auto" ref={elRoot}>
      {
        List && List.length > 0 && (
          <InfiniteScroll
      ...
      {!List || List.length === 0 && (
        <div className="h-full flex justify-center items-center flex-col">
          ...
          <div className="mt-3">Không có dịch vụ.</div>
        </div>
      )}
    </div >
  )
