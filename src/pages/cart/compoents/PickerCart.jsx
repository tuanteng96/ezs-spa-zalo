            ...
            item && (
              <div className="h-full">
                <div className="flex items-end px-3 pt-3 pb-4 border-b">
                  <div className="w-32">
                    <ImageLazy wrapperClassName="aspect-square !block" className="aspect-square object-cover w-full" effect="blur" src={toAbsolutePath(item?.ProdThumb)} />
                  </div>
                  <div className="pl-3">
                    <div className="font-semibold mb-2">{item?.ProdTitle}</div>
                    <div className="text-danger font-semibold">{formatString.formatVND(item.ToPay)}</div>
                  </div>
                </div>
                ...
                <div className="p-3">
                  <Button className={clsx('transition', watchQty > 0 ? '!bg-app' : '!bg-danger', addCartMutation.isLoading && "!bg-opacity-70")} fullWidth size="large" onClick={handleSubmit(onSubmit)} loading={addCartMutation.isLoading}>
