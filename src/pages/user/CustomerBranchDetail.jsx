...
import { Button, Icon, Page, Select, Text, useNavigate } from "zmp-ui";
import { openWebview } from "zmp-sdk";
import { Link, useLocation } from "react-router-dom";

const CustomerBranchDetail = () => {
  ...
  const openMaps = (item) => {
    openWebview({
      url: `https://www.google.com/maps/dir/?api=1&destination=${state?.Desc.split(
        " ",
      ).join("+")}`,
    });
  };
    ...
    <Page className="page !pb-0" hideScrollbar>
      <div className="navbar fixed top-0 left-0 min-w-[100vw] max-w-[100vw] z-[999] bg-white">
        <div className="w-2/3 relative flex items-center h-full pl-10">
          ...
          <Text.Title className="text-app">{state?.Title}</Text.Title>
        </div>
      </div>
      <div className="h-full flex flex-col">
        ...
        <div className="pb-safe-bottom bg-white p-4">
          <Button type="button" fullWidth onClick={openMaps}>
            Chỉ đường
          </Button>
          {/* <div className="mt-2.5">
            <Button fullWidth variant="secondary">Liên hệ</Button>
          </div> */}
        </div>
      </div>
    </Page>
