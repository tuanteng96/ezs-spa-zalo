    ...
    <>
      ...
      {createPortal(
        <Sheet visible={visible} onClose={() => setVisible(false)} autoHeight>
          <div>
            <div className="p-3 uppercase font-bold text-lg border-b border-b-4 text-app">{item.ten}</div>
            {
            ...
            <div className="p-3 border-b">
              ...
              {
                (item.gia_tri_chi_tieu_dv > 0 || item.gia_tri_chi_tieu_sp > 0) && (
                  <div>
                    <span>
                      Sản phẩm :
                      <span className="pl-1 font-bold text-base">
            ...
            <div className="p-3 border-b">
              ...
              </div>
              {
                ...
                (item.gioi_han_dv !== 0 || item.gioi_han_sp !== 0 || item.gia_tri_chi_tieu_dv !== 0 || item.gia_tri_chi_tieu_sp !== 0) && (
                  <div>
                    <span>
                      Sản phẩm :
                      <span className="pl-1 font-bold text-base">
            ...
            <div className="p-3 border-b border-b-4">
              ...
              {
                item.gia_tri_chi_tieu_sp - item.su_dung_sp > 0 || item.gia_tri_chi_tieu_dv - item.su_dung_dv && (
                  <div>
                    <span>
                      Sản phẩm :
                      <span className="pl-1 font-bold text-base">
                        {formatString.formatVND(item.gia_tri_chi_tieu_sp - item.su_dung_sp)}
                      </span>
                    </span>
                    <span className="pl-3">
                      Dịch vụ :
                      <span className="pl-1 font-bold text-base">
                        {formatString.formatVND(item.gia_tri_chi_tieu_dv - item.su_dung_dv)}
                      </span>
                    </span>
                  </div>
                )
              ...
              <div className="relative mt-5">
                ...
                )}
                {
                  !isLoading && (
                    <>
                      {!data || data.length === 0 && (
                        <div className="text-muted text-base">Thẻ tiền "Chưa" được sử dụng.</div>
                      )}
                      {
                        data && data.length > 0 && (
                          <ul className="pl-7 before:border-l before:border-[#cccbcd] before:border-dashed before:content-[''] before:h-full before:left-2 before:absolute">
                            {
                              data.map((item, index) => (
                                <li
                                  className={clsx("mb-6 relative before:content-[''] before:rounded-full before:w-[10px] before:h-[10px] before:absolute before:-left-[24px] before:top-[6px]", item.gia_tri > 0 ? "before:bg-success" : "before:bg-danger")}
                                  key={index}
                                >
                                  <div className="relative">
                                    <div className={clsx("font-bold text-base", item.gia_tri > 0 ? "text-success" : "text-danger")}>
                                      <span className="pr-1">{item.gia_tri > 0 ? "+" : "-"}</span>
                                  ...
                                  <div className="note absolute bottom-0 right-0 w-7/12 text-end">{item.san_pham}</div>
                                </li>
                              ))}
                          </ul>
                        )
