...
export const OrderItem = ({ item }) => {
  ...
  let TotalProd = item?.Items && item?.Items.reduce((sum, { Qty }) => sum + Qty, 0)
    ...
    <div className="bg-white mt-1.5">
      <div className="px-3 py-3 flex justify-between">
        ...
        <div className="flex items-center">
          <div className="text-sm">{moment(item.OrderDate).format("HH:mm DD-MM-YYYY")}</div>
      ...
      <PickerOrder item={item} TotalDebt={TotalDebt}>
        {({ open }) => (
          <div className="p-3 flex justify-between border-b items-center cursor-pointer" onClick={open}>
            <div className="text-muted text-sm">
              {TotalProd} mặt hàng
            </div>
            <div className="flex items-center text-sm">
              <span>Thành tiền : </span>
              <span className="text-app pl-2 font-semibold">{formatString.formatVND(item.ToPay)}</span>
        ...
        <PickerOrderPayted item={item}>
          {
            ({ open }) => (
              <button
                onClick={open}
                className={clsx('bg-app text-white h-11 px-4 rounded', !(item.Status !== 'cancel' && TotalDebt > 0) && 'opacity-50')}
                type="button"
