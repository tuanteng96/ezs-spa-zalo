...
const CustomerOrders = () => {
  ...
  const { data, isLoading } = useQuery({
    ...
    onSuccess: (data) => {
      ...
      const index = data.findIndex(x => moment().format("DD-MM-YYYY") === moment(x.dayFull).format("DD-MM-YYYY"));
      if (index > -1) {
            ...
            <>
              {
                !data || data.length === 0 && (
                  <div className="flex flex-col justify-center items-center h-5/6">
                    ...
                    <div className="font-bold text-lg mb-px">"Không" có nhật kí nào rồi !</div>
              ...
              {
                data && data.length > 0 && (
                  <Tabs className="tab-scrollbar" activeKey={TabActive} onChange={(e) => setTabActive(e)} scrollable>
                    {
                      data.map((item, index) => (
                        <Tabs.Tab key={item.day} label={"Ngày " + moment(item.dayFull).format("DD-MM-YYYY")}>
