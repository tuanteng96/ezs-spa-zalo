  ...
  return (
    <Page className="page !pb-0 !h-full !overflow-hidden" hideScrollbar>
      ...
      <div className="h-full border-t">
        <Tabs className="tab-scrollbar" activeKey={queryConfig.Type} onChange={onChangeTabs}>
            ...
            <div className="h-full flex flex-col">
              <div className="bg-white min-h-[150px] pt-7 flex flex-col flex-column items-center relative">
                <div className="text-danger text-2xl font-bold">
                  {
                    isLoadingWallet ? <div className="h-5 bg-gray-200 animate-pulse rounded-lg w-40"></div> : formatString.formatVNDPositive(WalletCard?.Wallet?.TotalWallet)
                  }
                </div>
                <div className="text-base font-medium mt-px text-[#929292]">Tổng Ví</div>
                    ...
                    <div className="grid grid-cols-2 shadow-5xl bg-white rounded-md p-3">
                      <div className="text-center py-1">
                        <div className="font-bold text-base text-success flex flex-col items-center">
                          {isLoadingWallet ? <div className="h-3 bg-gray-200 rounded-lg w-28 animate-pulse"></div> : formatString.formatVNDPositive(WalletCard?.Wallet?.DemonsWallet)}
                        </div>
                        <div className="text-[#929292] mt-px text-sm">Ví khả dụng</div>
                      </div>
                      <div className="text-center py-1">
                        <div className="font-bold text-base text-warning flex flex-col items-center">
                          {isLoadingWallet ? <div className="h-3 bg-gray-200 rounded-lg w-28 animate-pulse"></div> : formatString.formatVNDPositive(WalletCard?.Wallet?.TotalWallet - WalletCard?.Wallet?.DemonsWallet)}
                        </div>
                        <div className="text-[#929292] mt-px text-sm">Chờ xử lý</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="relative">
                    ...
                    <ul className="pl-7 animate-pulse before:border-l before:border-[#cccbcd] before:border-dashed before:content-[''] before:h-full before:left-2 before:absolute">
                      {
                        Array(2).fill().map((_, index) => (
                          <li
                            className={clsx("mb-6 relative before:content-[''] before:rounded-full before:w-[10px] before:h-[10px] before:absolute before:-left-[24px] before:top-[6px]", index > 0 ? "before:bg-success" : "before:bg-danger")}
                            key={index}
                          >
                            <div className="relative">
                              <div className={clsx("font-bold text-base", index > 0 ? "text-success" : "text-danger")}>
                  ...
                  )}
                  {
                    !isLoadingWallet && (
                      <>
                        {
                          !WalletCard?.Wallet?.HistoryUse || WalletCard?.Wallet?.HistoryUse.length === 0 && (
                            <div className="flex flex-col items-center px-5 py-12">
                              <svg className="w-16 mb-5" viewBox="0 0 56 60" xmlns="http://www.w3.org/2000/svg">
                                <g fillRule="evenodd">
                                  <path
                              ...
                              <div className="font-bold text-lg mb-px">"Hổng" có lịch sử sử dụng ví !</div>
                        ...
                        }
                        {
                          WalletCard?.Wallet?.HistoryUse && WalletCard?.Wallet?.HistoryUse.length > 0 && (
                            <ul className="pl-7 before:border-l before:border-[#cccbcd] before:border-dashed before:content-[''] before:h-full before:left-2 before:absolute">
                              {
                                WalletCard?.Wallet?.HistoryUse.map((item, index) => (
                                  <li
                                    className={clsx("mb-6 relative before:content-[''] before:rounded-full before:w-[10px] before:h-[10px] before:absolute before:-left-[24px] before:top-[6px]", item.Value > 0 ? "before:bg-success" : "before:bg-danger")}
                                    key={index}
                                  >
                                    <div className="relative">
                                      <div className={clsx("font-bold text-base", item.Value > 0 ? "text-success" : "text-danger")}>
                                        <span className="pr-1">{item.Value > 0 ? "+" : "-"}</span>
                                        {formatString.formatVNDPositive(item.Value)}
                                      </div>
                                      <div className="mt-2 text-muted">
                                        {moment(item.CreateDate).fromNow()}
                                      </div>
                                    </div>
                                    <div className="note absolute bottom-0 right-0">{VietnamesType(item)}</div>
                                  </li>
                                ))
                              }
                            </ul>
                  ...
                  <>
                    {
                      !data || data.length === 0 && (
                        <div className="flex flex-col items-center px-5 py-12">
                          ...
                          <div className="font-bold text-lg mb-px">"Hổng" có thẻ tiền nào ?</div>
